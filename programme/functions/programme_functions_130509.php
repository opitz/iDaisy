...
//	get assessment unit stint and hours
		if($au_id > 0)
		{
			$row['Norm AU Stint'] =  number_format(au_norm_stint($ay_id, $au_id),2);
			...
			$row['Norm AU Hours'] =  number_format(au_norm_hours($ay_id, $au_id),2);
			$row['Actual AU Hours'] =  number_format(au_hours($ay_id, $au_id),2);
		}
		else
		{
			$row['Norm AU Stint'] =  number_format(0,2);
			$row['Actual AU Stint'] =  number_format(0,2);
			$row['Actual AU Stint Share'] =  number_format(0,2);
			$row['Norm AU  Hours'] =  number_format(0,2);
			$row['Actual AU Hours'] =  number_format(0,2);
		}
...
//	amend Degree Programme percentage
		if($au_dp_students > 0) 
		{
			...
			$row['DP Stint'] = number_format($au_dp_students / $au_students * $row['Actual AU Stint'], 2);
...
//	get assessment unit stint and hours
			...
			$totals_row['Norm AU Hours'] =  number_format(au_norm_hours_share($ay_id, $prev_au_id),2);
			$totals_row['Actual AU Hours'] =  number_format(au_hours_share($ay_id, $prev_au_id),2);
...
//	get degree programme students
			if($au_id > 0)
			{
				...
				$au_dp_students = get_prog_au_students($ay_id, $prev_au_id, $dp_id);	// get the number of students enrolled into a given AU and DP
			}
			else
			{
				$au_students =  0;
...
//	summarize AU and DP stint for each providing department	
	if($table) foreach($table AS $row)
	{
		if($row['AU_ID'] > 0)
		{
			...
			$sum_au_stint_per_dept[$row['AU provided by']] = $sum_au_stint_per_dept[$row['AU provided by']] + $row['Actual AU Stint'];
			$sum_au_stint_share_per_dept[$row['AU provided by']] = $sum_au_stint_share_per_dept[$row['AU provided by']] + $row['Actual AU Stint Share'];

			$sum_norm_au_hours_per_dept[$row['AU provided by']] = $sum_norm_au_hours_per_dept[$row['AU provided by']] + $row['Norm AU Hours'];
			$sum_au_hours_per_dept[$row['AU provided by']] = $sum_au_hours_per_dept[$row['AU provided by']] + $row['Actual AU Hours'];

			$sum_dp_stint_per_dept[$row['AU provided by']] = $sum_dp_stint_per_dept[$row['AU provided by']] + $row['DP Stint'];	
			...
			$au_stint_sum = $au_stint_sum + $row['Actual AU Stint'];
			$au_stint_share_sum = $au_stint_share_sum + $row['Actual AU Stint Share'];

			$au_norm_hours_sum = $au_norm_hours_sum + $row['Norm AU Hours'];
			$au_hours_sum = $au_hours_sum + $row['Actual AU Hours'];
...
//	now amend the table with a summary line for each providing department
	...
	foreach($departments AS $department)
	{
		...
		$row['Actual AU Stint'] = $sum_au_stint_per_dept[$department];
		$row['Actual AU Stint Share'] = $sum_au_stint_share_per_dept[$department];
		$row['Norm AU Hours'] = $sum_norm_au_hours_per_dept[$department];
		$row['Actual AU Hours'] = $sum_au_hours_per_dept[$department];
		$row['DP Stint'] = $sum_dp_stint_per_dept[$department];
	...
	IF($_POST['excel_export'])
	{
		...
		$row['Actual AU Stint'] = $au_stint_sum;
		$row['Actual AU Stint Share'] = $au_stint_share_sum;
		$row['Norm AU Hours'] = $au_norm_hours_sum;
		$row['Actual AU Hours'] = $au_hours_sum;
		$row['DP Stint'] = $dp_stint_sum;
	...
	else
	{
		...
		$row['Actual AU Stint'] = "<B><U>" . $au_stint_sum . "</U></B>";
		$row['Actual AU Stint Share'] = "<B><U>" . $au_stint_share_sum . "</U></B>";
		$row['Norm AU Hours'] = "<B><U>" . $au_norm_hours_sum . "</U></B>";
		$row['Actual AU Hours'] = "<B><U>" . $au_hours_sum . "</U></B>";
		$row['DP Stint'] = "<B><U>" . $dp_stint_sum . "</U></B>";
...
function amend_stint_per_student($table)
{
	...
	if($table) foreach($table AS $row)
	{
		if($row['Students'] > 0) $row['Actual stint / student'] = number_format($row['Actual AU Stint'] / $row['Students'],2);
