...
//	last changes: M.Opitz | 2013-07-01
//
//==================================================================================================

$version = '120315.1';
$version = '120621.1';		// added term code input
$version = '121220.1';		// dealing with new Employee Numbers from HRIS
$version = '121221.3';		// Version with web interface
$version = '130111.1';		// added security
$version = '130322.1';		// if in HT copy back all MSc-Thesis supervison to the previous MT
$version = 'D3-130604.1';	// version for DAISY3
$version = 'D3-130625.1';	// now insterint startdate and created_at
$version = 'D3-130701.1';	// now allowing COMPLETED students

include '../includes/opendb.php';
include '../includes/common_functions.php';
include '../includes/common_DAISY_functions.php';

print "<B>DAISY update</B> |  'Supervision' data from OSS via Data Warehouse  v.$version";
print "<HR>";
...
//	if supervison data is imported from a Hilary Term copy MSc-Thesis supervision back into the previous Michaelmas Term
		...
		{
			$prev_mt_code = 'MT' . mysql_real_escape_string(substr($term_code,2,2) - 1);
//	delete any exiting MSc supervision records for that Michaelmas Term
			delete_msc_thesis_supervision($prev_mt_code);
//	now copy back from HT to MT
			copy_msc_thesis_supervision($term_code, $prev_mt_code);
		}	
...
//	$relenddate = get_term_startdate($term_code);
	...
	$query = "
		...
		WHERE 
			...
			AND (att.progattmptstatus = 'ENROLLED' OR att.progattmptstatus = 'COMPLETED')
...
//	$query = "SELECT sub_unit_code FROM SubUnit WHERE department_code = '$department_code'";
	$query = "SELECT su.sub_unit_code FROM SubUnit su INNER JOIN Department d ON d.id = su.department_id WHERE d.department_code = '$department_code'";
	$result = mysql_query($query) or die ("Could not execute query: $query " . mysql_error());
	$result_row = mysql_fetch_assoc($result);
...
//			$query = "SELECT department_code FROM SubUnit WHERE sub_unit_code = '$sub_unit_code'";
			$query = "SELECT d.department_code FROM Department d INNER JOIN SubUnit su ON su.department_id = d.id WHERE su.sub_unit_code = '$sub_unit_code'";
