...
//	print start_row(0) . "Code:" . new_column(0) . "<input type='text' name = 'dp_code_q' value='".$_POST['dp_code_q']."' size=50>" . end_row();		
	if(!$_POST['department_code'] AND current_user_is_in_DAISY_user_group('Overseer')) print start_row(0) . "Title:" . new_column(0) . "<input type='text' name = 'dp_title_q' value='".$_POST['dp_title_q']."' size=50>" . end_row();		
	else print start_row(0) . "Title:" . new_column(0) . degree_programme_options() . end_row();		
	print "</TABLE>";
...
//	build the query using the input
	if(!$query)
	{
		$query = "
			SELECT DISTINCT
...
//	Show a department column only when no department was selected in for the query
		...
		if($_POST['ay_id'] > 0) $query = $query .", COUNT(DISTINCT sdp.student_id) AS 'Students'";
		$query = $query . "
		...
		if($_POST['ay_id'] > 0) $query = $query ."LEFT JOIN StudentDegreeProgramme sdp ON sdp.degree_programme_id = dp.id AND sdp.status = 'ENROLLED' AND sdp.academic_year_id = ".$_POST['ay_id'];
		$query = $query . "
		...
		if($_POST['ay_id'] > 0) $query = $query."GROUP BY d.department_code, dp.degree_programme_code ";
...
//	if(1 == 1)
	{
		...
		do_the_miracle_solution_without_the_data_will_not_be_exported_and_I_have_no_clue_why();
	
//	if($_POST['show_programme_units']) show_programme_units_by_year($dp_data['id']);
	show_programme_units_by_year($dp_data['id']);
}
...
//--------------------------------------------------------------------------------------------------------------
function do_the_miracle_solution_without_the_data_will_not_be_exported_and_I_have_no_clue_why()
//
{
...
//	get the max year of student for any student enrolled or completed for the given degree programme
	...
	foreach($ac_years AS $ac_year)
	{
		...
		if(has_enrolled_students($ay_id, $dp_id))
		{
			...
			$query ="
				SELECT
				sdp.year_of_student AS 'Year of Student',
				...
				GROUP BY sdp.year_of_student
			";
			if($_POST['debug'] == 'query') d_print($query);
			$dp_st_yos = get_data($query);
		
			if($dp_st_yos) foreach($dp_st_yos AS $dp_st)
			{
				$row["Stud. Year " . $dp_st['Year of Student']] = $dp_st['Students'];
			}
			$table[] = $row;
		}
	}
...
//	for each assessment unit amend the data  *********************************************************************************
		$table = amend_programme_units_by_year($table);
...
//--------------------------------------------------------------------------------------------------
function amend_programme_units_by_year($table)
//	for each assessment unit amend the data
{
	$ay_id = $_GET['ay_id'];								// get academic_year_id
	if(!$ay_id) $ay_id = $_POST['ay_id'];
...
//	get the degree programmes that have students in any assessment unit of this degree programme
	if($_POST['show_students'])
//	if (1==1)
	{
		$row2=array();
		$student_depts = get_all_student_depts($ay_id, $dp_id);
		if($student_depts) foreach($student_depts AS $st_dept)
		{
			if($st_dept['DP_ID'] == $dp_id) $column_title = '<FONT COLOR =#FF6600>'.$st_dept['Code'].' - '.$st_dept['Degree Programme'].'</FONT>';
			else $column_title = $st_dept['Code'].' - '.$st_dept['Degree Programme'];
			$row2["$column_title"] = '';
		}
	} 
...
//		if (1==1)
		{
			$row = array_merge($row, $row2);	//amend the row with the fields for possible degree programme student numbers
//	get the degree programmes that have students in the given assessment unit
			$au_student_depts = get_au_student_depts($ay_id, $au_id);
			if($au_student_depts) foreach($au_student_depts AS $st_dept)
			{
				if($st_dept['DP_ID'] == $dp_id) $column_title = '<FONT COLOR =#FF6600>'.$st_dept['Code'].' - '.$st_dept['Degree Programme'].'</FONT>';
				else $column_title = $st_dept['Code'].' - '.$st_dept['Degree Programme'];
				$sdp_id = $st_dept['DP_ID'];
				$dp_students = get_prog_au_students($ay_id, $au_id, $sdp_id);
				$row["$column_title"] = $dp_students;
			}
		}
...
//		if($_POST['show_students'])
		if (1==1)
		{
			$prog_students = get_prog_au_students($ay_id, $au_id, $dp_id);			// get the number of prog students enrolled into AU
			$all_students = get_all_au_students($ay_id, $au_id);						// get the number of all students enrolled into AU
			$row['AU Stint'] = '';
...
//	get the related teaching instances for each teaching component
		...
		{
			$result_array =  get_au_instances($row, $sums, $prog_students, $all_students);

		} else // no instances
		{
			$result_array =  get_no_au_instances($row, $sums, $prog_students, $all_students);
		}
//		$new_table[] = $result_array['row'];
		$row = $result_array['row'];

		array_shift($row);
		if($_POST['show_components']) array_shift($row);
		$new_table[] = $row;
		$sums = $result_array['sums'];
	}
//	add a sum row
	$new_table = add_sums_programme_units_by_year($new_table, $sums);

	return $new_table;
}

//--------------------------------------------------------------------------------------------------
function add_sums_programme_units_by_year($table, $sums)
//	add a sum row
{
	$row=$table[0];
	if($sums)
...
//	now do the sums line		
		$departments = array_keys($sums);
		$dp_stint_sum = 0;
		$au_stint_sum = 0;
		foreach($departments AS $department)
		{
			$row['Department'] = $department;
			$row['AU Stint'] = $sums[$department]['AU Stint'];
			$row['DP Stint'] = $sums[$department]['DP Stint'];
			$table[] = $row;
		
			$dp_stint_sum = $dp_stint_sum + $row['DP Stint'];
			$au_stint_sum = $au_stint_sum + $row['AU Stint'];
		}
		$row['Department'] = '<B>Total:</B>';
		$row['AU Stint'] = "<U><B>".$au_stint_sum."</B></U>";
		$row['DP Stint'] = "<U><B>".$dp_stint_sum."</B></U>";
		$table[] = $row;		
	}
	return $table;
}
...
//--------------------------------------------------------------------------------------------------
function get_au_instances($row, $sums, $prog_students, $all_students)
//	show the instances for a given assessment unit
{
	$ay_id = $_POST['ay_id'];
	$dp_id = $_POST['dp_id'];
...
//d_print($query);			
	...
	{
		if($_POST['show_hours']) $row['Norm Hours'] = $component['norm_hours'];
...
//		$row['Location'] = $component['location_name'];
		$row['AU Stint'] = $component['stint'];
		if($_POST['show_hours']) $row['Hours'] = $component['hours'];
...
//		if($_POST['show_students'])
		if (1==1)
		{
			if($all_students)
			{
				$row['DP. %'] = number_format($prog_students / $all_students * 100,2);
				$row['DP Stint'] = number_format($row['AU Stint'] / $all_students * $prog_students,2);
			} else
			{
				$row['DP. %'] = number_format(0,2);
				$row['DP Stint'] = number_format(0,2);
			}
		} 
//	write sums
		$sums[$row['Department']]['AU Stint'] = $sums[$row['Department']]['AU Stint'] + $row['AU Stint'];
		$sums[$row['Department']]['DP Stint'] = $sums[$row['Department']]['DP Stint'] + $row['DP Stint'];
		$stint_sum = $stint_sum + $row['AU Stint'];
	} else 	// no components found - fill in empty fields
	{
		if($_POST['show_hours']) $row['Norm Hours'] = '';
		$row['Given Sessions'] = '';
		$row['Lecturer'] = '';
		$row['Term'] = '';
//		$row['Session Dates'] = '';
...
//		if($_POST['show_students'])
	...
	$result_array = array('row' => $row, 'sums' => $sums);
	return $result_array;
} 

//--------------------------------------------------------------------------------------------------
function get_au_instances0($row, $sums, $prog_students, $all_students)
//	show the instances for a given assessment unit
{
	$ay_id = $_POST['ay_id'];
	$dp_id = $_POST['dp_id'];
...
//--------------------------------------------------------------------------------------------------
function get_no_au_instances($row, $sums, $prog_students, $all_students)
//	show no instances for a given assessment unit
{
	$ay_id = $_GET['ay_id'];								// get academic_year_id
	if(!$ay_id) $ay_id = $_POST['ay_id'];
	$dp_id = $_GET["dp_id"];								// get programme ID dp_id
...
//	get the total stint value used by an assessment unit in a given academic year
	$query = "
		SELECT
		FORMAT(SUM(ti.sessions * ti.percentage / 100 * IF(ti.stint_override > 0, ti.stint_override, IF(tcau.stint_override > 0, tcau.stint_override, tctt.stint))),2) AS 'Stint'
...
//	if showing components get the total stint value used by a teaching component in a given academic year
	if($_POST['show_components'] AND $tc_id) $query = "
		...
		TeachingComponentType tct 
		INNER JOIN TeachingComponentTypeTariff tctt ON tctt.teaching_component_type_id = tct.id AND tctt.academic_year_id = $ay_id
	
		WHERE t.academic_year_id = $ay_id
		AND tc.id = $tc_id
		AND tct.id = (SELECT teaching_component_type_id FROM TeachingComponentAssessmentUnit WHERE teaching_component_id = $tc_id LIMIT 1) 
	";

	$result = get_data($query);
	$item = $result[0];
	$stint_total = $item['Stint'];
	$row['AU Stint'] = number_format($stint_total,2);
	if($prog_students AND $all_students)
	{
		$row['DP %'] = number_format($prog_students / $all_students * 100,2);
		$row['DP Stint'] = number_format($stint_total / $all_students * $prog_students,2);
	} else
	{
		$row['DP %'] = number_format(0,2);
		$row['DP Stint'] = number_format(0,2);
...
//--------------------------------------------------------------------------------------------------
function get_no_au_instances0($row, $sums, $prog_students, $all_students)
//	show no instances for a given assessment unit
{
	$ay_id = $_GET['ay_id'];								// get academic_year_id
	if(!$ay_id) $ay_id = $_POST['ay_id'];
...
//--------------------------------------------------------------------------------------------------
function get_all_au_students0($ay_id, $au_id)
//	get the number of enrolled students for a given assessment unit and academic year
{
	$query = "
		SELECT
...
//--------------------------------------------------------------------------------------------------
function get_prog_au_students0($ay_id, $au_id, $dp_id)
...
//--------------------------------------------------------------------------------------------------
function get_all_au_students($ay_id, $au_id)
//	get the number of enrolled students for a given assessment unit and academic year
{
	$query = "
		SELECT
...
//--------------------------------------------------------------------------------------------------
function get_prog_au_students($ay_id, $au_id, $dp_id)
//	get the number of  students enrolled to a given assessment unit and of a given degree programme  in a given academic year
{
	$query = "
		SELECT
...
//--------------------------------------------------------------------------------------------------
function get_all_student_depts($ay_id, $dp_id)
//	returns table containing all degree programmes that have students in any assessment unit related to a given degree programme
// 	this 1st version takes about 30 seconds for a given degree programme to compute
{
	$query = "
SELECT DISTINCT
dp.id AS 'DP_ID',
...
INNER JOIN StudentAssessmentUnit sau ON sau.assessment_unit_id = au.id AND sau.academic_year_id = audp.academic_year_id
...
//--------------------------------------------------------------------------------------------------
function get_au_student_depts($ay_id, $au_id)
//	returns table containing all degree programmes that have students in the given assessment unit
{
	$query = "
SELECT DISTINCT
dp.id AS 'DP_ID',
...
//--------------------------------------------------------------------------------------------------
function show_prog_teaching_details_by_year000($dp_id, $given_ay_id)
...
//----------------------------------------------------------------------------------------
function get_prog_teaching_stint0($dp_id, $ay_id)
//	get the teaching stint for an assessment unit for a give academic year
{
//	select the teaching data
	$query = "
...
//	write out all $_POST attributes as hidden input
	...
	if (programme_has_students($dp_id, $ay_id)) 
	{
		if($_POST['show_students'])
		{
			...
			$html = $html."<input type='submit' value='Hide Student per DP'>";
		} else
		{
			$html = $html."<input type='hidden' name='show_students' value=1>";
			$html = $html."<input type='submit' value='Show Student per DP'>";
		}
	} else
	{
		$html = $html."<input type='hidden' name='show_students' value=0>";
...
//	write out all $_POST attributes as hidden input
	...
	if($_POST['show_hours'])
	{
		$html = $html."<input type='hidden' name='show_hours' value=0>";
		$html = $html."<input type='submit' value='Hide Hours'>";
	} else
	{
		$html = $html."<input type='hidden' name='show_hours' value=1>";
		$html = $html."<input type='submit' value='Show Hours'>";
	}
