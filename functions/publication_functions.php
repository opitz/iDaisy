...
//	12-12-05	renamed options for report type
//	12-12-06	enabled filtering by title for REF reports
//	12-12-17	enabled Estimated REF Ranking in output for now - it will have to be disabled before upload to REF as it will reject unknown columns
//==================================================================================================
...
//	if(!$query AND ($department_code OR $fullname_q OR $forename_q OR $webauth_q OR $employee_nr_q))
	...
	$query = "
		...
		e.ref_unit_of_assessment AS 'unitOfAssessment',
		e.ref_multiple_submission AS 'multipleSubmission',
		'Overwrite' AS 'action',
		e.ref_hesa_staff_identifier AS 'hesaStaffIdentifier',
		e.opendoor_employee_code AS 'staffIdentifier',
		e.fullname AS 'author',
		rr.label AS 'outputNumber',
		pub.id AS 'outputIdentifier',
		SUBSTR(pubt.publication_type_name, LOCATE('(', pubt.publication_type_name) + 1, 1) AS 'outputType',
		pub.title AS 'title',
		pub.place AS 'place',
		publ.publisher_name AS 'publisher',
		IF(pub.volume_publication_id > 0, (SELECT pub1.title FROM Publication pub1 WHERE pub1.id = pub.volume_publication_id), IF(pub.volume_title = '' OR pub.volume_title IS NULL,j.journal_name,pub.volume_title)) AS 'volumeTitle',
#		j.journal_name AS 'Journal Volumexxx',
		LEFT(pub.journal_volume, 16) AS 'volume',
		LEFT(pub.issue, 16) AS 'issue',
		IF(SUBSTRING_INDEX(SUBSTRING_INDEX(pub.pagination, ' ', 1), '-', 1) REGEXP '^[0-9]$', SUBSTRING_INDEX(SUBSTRING_INDEX(pub.pagination, ' ', 1), '-', 1), SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(pub.pagination, ' ', -1), '.', -1), '-', 1)) as 'firstPage',
		pub.article_number AS 'articleNumber',
		pub.isbn AS 'isbn',
		j.issn_hard_copy AS 'issn',
		pub.doi AS 'doi',
		pub.patent_registration_number AS 'patentNumber',
		YEAR(pub.publicationdate) AS 'year',
		pub.estimated_ref_quality_rating AS 'Est. REF Qual. Rating',
		pub.url AS 'url',
		pub.output_media AS 'mediaOfOutput',
		(SELECT COUNT(pube2.id) FROM PublicationEmployee pube2 WHERE pub.id = pube2.publication_id) - 1 AS 'numberOfAdditionalAuthors',
		IF(pub.publication_status_id = 10, 'Yes', '') AS 'isPendingPublication',
		IF(pub.duplicate_output = 1, 'Yes', '') AS 'isDuplicateOutput',
		IF(pub.non_english = 1, 'Yes', '') AS 'isNonEnglishOutput',
		IF(pub.interdisciplinary = 1, 'Yes', '') AS 'isInterdisciplinary',
		IF(pub.propose_double_weighting = 1, 'Yes', '') AS 'proposeDoubleWeighting',
		pub.double_weighting_statement AS 'doubleWeightingStatement',
		IF(pub.reserve_output > 0 AND pub.reserve_output < 5, pub.reserve_output, '') AS 'reserveOutput',
		IF(pub.conflict_of_interest = 1, 'Yes', '') AS 'hasConflictOfInterest',
		pub.conflict_panel_members AS 'conflictedPanelMembers',
		IF(pub.is_cross_referral = 1, 'Yes', '') AS 'isOutputCrossReferred',
		IF(pub.cross_referral_UOA > 0 AND pub.cross_referral_UOA < 37, pub.cross_referral_UOA, '') AS 'crossReferToUoa',
		pub.additional_information AS 'additionalInformation',
		pub.english_abstract AS 'englishAbstract',
		pub.research_group AS 'researchGroup',
		IF(pub.is_sensitive = 1, 'Yes', '') AS 'isSensitive'
		
		
		FROM PublicationEmployee pube
		INNER JOIN Employee e ON e.id = pube.employee_id
		INNER JOIN Department d ON SUBSTRING(d.departmental_uoa, 1, 2) = e.ref_unit_of_assessment AND IF(SUBSTRING(d.departmental_uoa, 3, 1) = 'A' OR SUBSTRING(d.departmental_uoa, 3, 1) = 'B', SUBSTRING(d.departmental_uoa, 3, 1) = e.ref_multiple_submission, 1=1)
...
#		j.journal_name AS 'Journal Volumexxx',
	...
	if(!$query)
	{
		...
		if($report_type == 'General report: standard' OR $report_type == 'General report: detailed') $query = $query."
			psh.publisher_name AS 'Publisher',";
		if($report_type == 'General report: detailed') $query = $query."
...
//		$pub_id = array_shift($row);
		if(!$report_type == 'REF report' AND !$report_type == 'REF report compact') 
		{
			$pub_id = array_shift($row);
//	get the author(s)
			$query = "
...
//	if ($excel_export AND $ay_id) export2excel($new_table, $export_title);
	if ($table AND $excel_export) export2csv($new_table, "Publication_Report_");
...
{
	$report_type = $_POST['report_type'];					// get report_type
	if(!$report_type) $report_type = $_GET['report_type'];
