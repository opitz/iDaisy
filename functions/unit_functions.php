...
$version_cuf = "130228.1";			// using stint_override
...
//	select the data
	...
	$query = "
		...
		audp.unit_type as 'Type',
		audp.core_option as 'Core / Option',
		CONCAT(d.department_name,' (',dpd.percentage,')') AS 'Provided by (%)'
		
		FROM DegreeProgramme dp
		INNER JOIN AssessmentUnitDegreeProgramme audp ON audp.degree_programme_id = dp.id
		INNER JOIN AcademicYear ay ON ay.id = audp.academic_year_id
		
		INNER JOIN DegreeProgrammeDepartment dpd ON dpd.degree_programme_id = dp.id AND dpd.academic_year_id = $ay_id AND dpd.year_of_programme = 1
...
IF(ti.stint_override > 0, ti.stint_override, IF(tcau.stint_override > 0, tcau.stint_override, tctt.stint)) AS 'Sess. Stint',
...
//--------------------------------------------------------------------------------------------------
function show_unit_teaching_details_by_year0()
//	print teaching details for a given Assessment Unit ID
{
	$given_ay_id = $_GET['ay_id'];												// get selected academic_year_id
	if(!$given_ay_id) $given_ay_id = $_POST['ay_id'];
...
st.id AS 'ST_ID', ";
		...
		if($_POST['excel_export']) $query = $query . "CONCAT(st.surname, ', ', st.forename) AS 'Student', ";
		else $query = $query . "CONCAT('<A HREF=student/index.php?s_id=',st.id,'>',st.surname, ', ', st.forename,'</A>') AS 'Student', ";
...
//----------------------------------------------------------------------------------------
function get_unit_teaching_stint0($au_id, $ay_id)
//	get the teaching stint for an assessment unit for a give academic year
{
//	select the teaching data
	$query = "
