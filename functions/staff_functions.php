...
//	2013-03-21	various interface changes
//	2013-04-16	using start and end term in Leave records
//	2013-04-25	adding grade in staff list
//	2013-05-08	hiding staff from the casual payroll by default, checkbox added to include them
//	2013-05-31	using start_term_id in leave tables
//	2013-06-12	fixed bug in calculating ACTVness for an AY when post was started in mid-AY
//	2013-06-25	added start date of supervision
//==================================================================================================
...
//	display the option checkboxes
	print "<TABLE BORDER=0>";
	print start_row(250);		// 1st row, 1st column
		print "Show stint details:";
	print new_column(60);
		print html_checkbox('show_stint_details_list');

	print new_column(250);		// 1st row, 2nd column
		print  "Include non-academic staff:";
	print new_column(60);
		print html_checkbox('non_academic');
	print end_row();


	print start_row(0);		// 2nd row, 1st column
		print "Show full details:";
	print new_column(60);
		print html_checkbox('show_full_details_list');

	print new_column(0);	// 2nd row, 2nd column
	...
	print start_row(0);		// 3rd row, 1st column
		print "Show manually added staff only:";
	print new_column(0);
		print html_checkbox('manual_only');

	print new_column(0);	// 3rd row, 2nd column
		print "Include staff not ACTV this year:";
	print new_column(0);
		print html_checkbox('non_actv');
	print end_row();
	...
	if (current_user_is_in_DAISY_user_group('Divisional-Reporter')) 
	{
		...
		print new_column(0);
			print html_checkbox('joint_post_only');
	} else
	{
		print start_row(0);		// 4th row
		print new_column(0);
	...
	print new_column(0);	// 4th row, 2nd column
		print "Include staff on Casual Payroll:";
	print new_column(0);
		print html_checkbox('cas_pay');
	print end_row();


	print start_row(0);		// 5th row, 1st column
//		print "Show manually added staff only:";
	print new_column(0);
//		print html_checkbox('manual_only');

	print new_column(0);			//5th row, 2nd column
		 print "Include borrowed staff:";
	print new_column(0);
		print html_checkbox('include_borrowed_staff');
	print end_row();
...
*/
	...
	$query = $query."
		p.grade AS 'Grade', 
		";
	if(current_user_is_in_DAISY_user_group("Super-Administrator")) $query = $query."
		p.fte_department AS 'FTE', 
		";
/*
	if($_POST['excel_export']) $query = $query . "
		e.email AS 'Email', ";
...
*/
	...
	if(!$_POST['cas_pay']) $query = $query."AND p.grade != 'X99' ";
...
//	if(!$_POST['non_actv']) $query = $query."AND p.startdate <= '$ay_enddate' AND IF(YEAR(enddate) > 1980, p.enddate >= '$ay_startdate', 1=1) ";
	if(!$_POST['non_actv']) $query = $query."AND IF(YEAR(startdate) > 1980, p.startdate <= '$ay_enddate', 1=1) AND IF(YEAR(enddate) > 1980, p.enddate >= '$ay_startdate', 1=1) ";
...
//		else $query = $query."AND (SELECT COUNT(*) FROM Post p2 INNER JOIN Department d2 ON d2.id = p2.department_id WHERE p2.employee_id = e.id AND IF(p.person_status != '', p2.person_status = 'p.person_status', 1=1) AND d2.department_code LIKE '$department_code%') ";
	}
...
//	print_header('Academic Stint Report - Overview');
	...
	if($_POST['show_stint_details_list']) $header = 'Academic Stint Report - Stint Details';
	if($_POST['show_full_details_list']) $header = 'Academic Stint Report - Full Details';
	if(!$excel_export)
	{		
		print_header($header);
...
//			$row['Stint'] = "<B>".round($st_total,2)."</B>";
		...
		if($row['Grade'] == 'X99') 
			if($notes) $notes = $notes . ", Cas. Payroll";
			else $notes = "Cas. Payroll";
		$row['Remarks'] = $notes;
	
//		array_shift($row);		// get rid of the ID in the first column that is too ugly to display...
//		array_shift($row);		// get rid of the ID in the first column that is too ugly to display...
//		array_shift($row);		// get rid of the ID in the first column that is too ugly to display...
//		if(!$_POST['show_full_details_list'] AND !$_POST['show_stint_details_list']) array_shift($row);		// get rid of the ID in the first column that is too ugly to display...

		if($_POST['include_zero_stint'] OR !$ay_id ) $new_table[] = $row;
		else	 if($st_total) $new_table[] = $row;
	}

	if($_POST['show_full_details_list']) 							//	print full details for each staff member in the list
	{
		$_POST['show_post'] = 1;
		$_POST['show_teaching'] = 1;
		$_POST['show_supervision'] = 1;
		$_POST['show_leave'] = 1;
		$_POST['show_publication'] = 1;
		$_POST['show_office'] = 1;
		$_POST['show_committee'] = 1;
...
//		$table_width = array('Status' => 50, 'Notes' => 80, 'Red. Obligation' => 50, 'Teaching' => 50, 'Supervision' => 50, 'Stint' => 50, 'Balance' => 50);
	...
	elseif($_POST['show_stint_details_list']) 							//	print stint details for each staff member in the list
	{
		$_POST['show_post'] = 1;
		$_POST['show_teaching'] = 1;
		$_POST['show_supervision'] = 1;
...
*/
		if($dept_id)
		{
			$new_table = insert_totals($new_table);
		}


//	now do the output
		$new_table = cleanup($new_table,4);
		$table_width = array('Status' => 50, 'Notes' => 80, 'Red. Obligation' => 50, 'Teaching' => 50, 'Supervision' => 50, 'Stint' => 50, 'Balance' => 50);
...
//	inserts a total row for teaching columns
{
	$ay_id = $_POST['ay_id'];					// get academic_year_id
	...
	$total_stint_obligation = 0;
	$total_reduced_stint_obligation = 0;
	$total_teaching_stint = 0;
	$total_supervision_stint = 0;
	$total_balance = 0;
	...
	if($table) foreach($table AS $row)
	{
		if($row['D_ID'] == $dept_id)
		{
			$total_stint_obligation = $total_stint_obligation  + $row['Obligation'];
			$total_reduced_stint_obligation = $total_reduced_stint_obligation + $row['Red. Obligation'];
			$total_teaching_stint = $total_teaching_stint + $row['Teaching'];
			$total_supervision_stint = $total_supervision_stint + $row['Supervision'];
//			$total_balance = $total_balance + $row['Balance'];
			$total_balance = $total_balance + $row['Teaching'] + $row['Supervision'] - $row['Red. Obligation'];
		}
		$new_table[] = $row;
	}
	...
	IF($_POST['excel_export'])
	{
		$totals_row['Full Name'] = "Total:";
		$totals_row['Obligation'] = $total_stint_obligation;
		$totals_row['Red. Obligation'] = $total_reduced_stint_obligation;
		$totals_row['Teaching'] = $total_teaching_stint;
		$totals_row['Supervision'] = $total_supervision_stint;
		$totals_row['Balance'] = $total_balance;
	}
	else
	{
		$totals_row['Full Name'] = "<B><U>Total:</U></B>";
		$totals_row['Obligation'] = "<B><U>" . $total_stint_obligation . "</U></B>";
		$totals_row['Red. Obligation'] = "<B><U>" . $total_reduced_stint_obligation . "</U></B>";
		$totals_row['Teaching'] = "<B><U>" . $total_teaching_stint . "</U></B>";
		$totals_row['Supervision'] = "<B><U>" . $total_supervision_stint . "</U></B>";
		$totals_row['Balance'] = "<B><U>" . $total_balance . "</U></B>";
	}
...
//	 	print "<HR>";
	...
	{
		if (has_published($e_id)) show_publication_details($e_id);
		else show_no_pub();
	}

	if($_POST['show_office'])
	{
		if (is_office_holder($e_id)) show_offices($e_id);
		else show_no_office();
	}

	if($_POST['show_committee'])
	{
		if (is_cttee_member($e_id)) show_committees($e_id);
		else show_no_cttee();
	}

//	if($_POST['show_publication']) show_publication_details();
//	if($_POST['show_publication']) show_publication_details($e_id);
//	if($_POST['show_office']) show_offices();
//	if($_POST['show_office']) show_offices($e_id);
...
//	print "</TR><TR>";
	...
	if(!$excel_export) 
	{
		...
		if($e_data['old_opendoor_employee_code'] > 0 AND $e_data['old_opendoor_employee_code'] != $e_data['opendoor_employee_code']) print "<TD>".$e_data['opendoor_employee_code']." (was: ".$e_data['old_opendoor_employee_code'].")</TD>";
		else print "<TD>".$e_data['opendoor_employee_code']."</TD>";
...
//	now do for each post (= table row)
	if($table)
	{
		...
		{
//			if($row['Status'] == 'ACTV')
			if($row['Start Date'] < $ay_enddate AND (($row['End Date'] != '' AND $row['End Date'] > $ay_startdate) OR $row['End Date'] == '') )
			{
				$new_row = array();
				$p_id = $row['P_ID'];
				...
				if($ay_id > 0)
				{
					if(!$borrowed) 
					{
						...
						if($leave_reduction_factor) 
//							$row['Red. Obligation'] = round($stob - $stob * $leave_reduction_factor,2);
							$rstob = round($stob - $stob * $leave_reduction_factor,2);
						else
//							$row['Red. Obligation'] = $stob;
							$rstob = $stob;
						if($rstob < 0) $rstob = 0;
						$row['Red. Obligation'] = $rstob;
					}
...
//			if(current_user_is_in_DAISY_user_group("Overseer")) $query = $query.", CONCAT('<A HREF=https://daisy.socsci.ox.ac.uk/post/', p.id, '/edit TARGET=NEW>Edit</A>') AS 'DAISY'";
			...
			array_shift($row);		// get rid of the ID in the first column that is too ugly to display...
			array_shift($row);		// get rid of the ID in the first column that is too ugly to display...
			if(!isset($_POST['excel_export']) AND current_user_is_in_DAISY_user_group("Overseer")) $row['DAISY'] = "<A HREF=https://daisy.socsci.ox.ac.uk/post/$p_id/edit TARGET=NEW>Edit</A>";
			$new_table[] = $row;
		}
...
//		if($_POST['show_post'] AND (current_user_is_in_DAISY_user_group("Editor") OR !current_user_is_in_DAISY_user_group("Editor")))
		if($_POST['show_post'] )
		{
			...
			Only the department(s) the staff member has a post with will see the complete report.<P /></FONT>";
...
//	Display a DAISY edit link for each instance
			if(!$_POST['excel_export'] AND current_user_is_in_DAISY_user_group("Overseer")) $row['DAISY'] = "<A HREF=https://daisy.socsci.ox.ac.uk/teaching_instance/$ti_id/edit TARGET=NEW>Edit</A>";
			array_shift($row);		// get rid of the ID in the first column that is too ugly to display...
			array_shift($row);		// get rid of the ID in the first column that is too ugly to display...
			array_shift($row);		// get rid of the ID in the first column that is too ugly to display...
			array_shift($row);		// get rid of the ID in the first column that is too ugly to display...
...
//	define column width in table
	$table_width = array('Department' => 300, 'Student' => 450, 'Type' => 150, 'Since' => 100, 'OGY' => 50, 'Term' => 50, 'MT (%)' => 60, 'HT (%)' => 60, 'TT (%)' => 60, 'Stint' => 60);
...
//			else
			...
			if(current_user_is_in_DAISY_user_group("Overseer")) $query = $query."CONCAT('<A HREF=student/index.php?s_id=',st.id,'>',st.surname, ', ', st.forename,'</A>') AS 'Student', ";
			else $query = $query."CONCAT(st.surname, ', ', st.forename) AS 'Student', ";
			$query = $query."
			svt.supervision_type_code AS 'Type',
			DATE(sv.startdate) AS 'Since',
...
//			$sum_stint["$dept"] = $sum_stint["$dept"] + $student_supervision_stint;
//	link to DAISY
			if(!$_POST['excel_export'] AND current_user_is_in_DAISY_user_group("Overseer")) $row['DAISY'] = "<A HREF=https://daisy.socsci.ox.ac.uk/supervision/".$sv_id."/edit TARGET=NEW>Edit</A>";
...
//	select the data
		$query = "
			...
			if(!$_POST['excel_export'] AND current_user_is_in_DAISY_user_group("Overseer")) $query = $query.", 
...
#			AND st.academic_year_id = $ay_id
			AND st.startdate < ay.enddate
			AND et.enddate > ay.startdate
			ORDER BY d.department_code, st.startdate
			";
...
//---------------------------------------------------------------------------------------------------------------
function show_no_pub()
//	print no leave
{
	print "<HR><H3>No Publication</H3>";
}

//---------------------------------------------------------------------------------------------------------------
function show_no_office()
//	print no leave
{
	print "<HR><H3>No Office Holding</H3>";
}

//---------------------------------------------------------------------------------------------------------------
function show_no_cttee()
//	print no leave
{
	print "<HR><H3>No Committee Membership</H3>";
}
...
AND t.startdate < ay.enddate
AND t.enddate > ay.startdate
AND t.startdate >= st.startdate
AND t.enddate <= et.enddate
AND LEFT(t.term_code,2) != 'NA'
...
{
	if($ay_id > 0) $query = "SELECT * FROM EmployeeAcademicLeave eal INNER JOIN Term t ON t.id = eal.start_term_id WHERE eal.employee_id = $e_id AND t.academic_year_id = $ay_id";
