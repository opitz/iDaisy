...
// 2012-06-07 - 1. Version    ==   Happy birthday, Winnie & Mum!
...
// 2013-06-28 - addedd get_mssql_data()
// 2013-07-01 - addedd read_record()
// 2013-07-30 - set $_POST['ay_id'] in print_header() if not already done so
// 2013-08-12 - allow to set debug options in ini file and display GET and POST before the header
// 2013-08-13 - get_data() now handles boolean return values from quieries
//
//--------------------------------------------------------------------------------------------------------------
...
//	if no academic year is selected in the first place propose the current academic year as a selection
	if(!$_POST['ay_id']) $_POST['ay_id'] = get_current_academic_year_id();

	$ay_label = get_academic_year_label($_POST['ay_id']);
	

	print '	<head>
		<script language="javascript" src="/iDaisy/basic/calendar/calendar.js"></script>
		</head>';


//	show debug parameters if debug option is set
	$debug = $params['debug'];
	if(strlen($debug) > 0)
	{
		dget();
...
//	print "<FONT face='Arial'><B><FONT COLOR=#FF6600>i</FONT>DAISY | $title ($ay_label)</B> ";
	if ($_SERVER['HTTP_HOST'] == 'daisy.socsci.ox.ac.uk') print "<FONT face='Arial'><B><FONT COLOR=#FF6600>i</FONT><A HREF=https://daisy.socsci.ox.ac.uk/ COLOR=#555588 STYLE=TEXT-DECORATION:NONE>DAISY</A> | $title ($ay_label)</B> ";
	else print "<FONT face='Arial'><B><FONT COLOR=#FF6600>i</FONT><A HREF=http://git-sfproject.socsci.ox.ac.uk/overseer_dev.php/ STYLE=TEXT-DECORATION:NONE>DAISY</A> | $title</B> ";
	if($department_code) print " for $department ($department_code) ";
	print " | Academic Year: $ay_label";
	print "</FONT> ";
...
//		print "<BR><FONT face='Arial' COLOR=#766666><EM>$date, run by <B>$fullname</B> ($webauth_code)";
		if(current_user_is_in_DAISY_user_group("Editor")) print "<BR><FONT face='Arial' COLOR=#766666><EM>$date, run by <B><A HREF=academic.php?e_id=$e_id STYLE=TEXT-DECORATION:NONE>$fullname</A></B> ($webauth_code)";
		else print "<BR><FONT face='Arial' COLOR=#766666><EM>$date, run by <B>$fullname</B> ($webauth_code)";
...
//	if(isset($_POST['deb'])) if($_POST['deb'] == 'query') d_print($query);
	...
	{
		$row['Query Result'] = $result;
		$table[] = $row;
	}
	elseif($result) while ($row = mysql_fetch_assoc($result))	
	{
		$table[] = $row;
	}
...
//--------------------------------------------------------------------------------------------------------------
function get_mssql_data($query)
//	do the MSSQL query and store the result in a table that is returned
{

	$result = mssql_query($query) or die ("Could not execute query: " . d_print($query) . "Error = " . mysql_error());	

	if($result) while ($row = mssql_fetch_assoc($result))	
	{
		$table[] = $row;
	}
...
//--------------------------------------------------------------------------------------------------------------
function geti_data($conni, $query)
//	do the query and store the result in a table that is returned
{
//	if $_POST['deb'] == 'query' show the query
//	if(isset($_POST['deb'])) if($_POST['deb'] == 'query') d_print($query);

	$result = mysqli_query($conni, $query) or die ("Could not execute query: " . d_print($query) . "Error = " . mysql_error());	
...
// shows record with the given ID from a given table
{
...
function show_footer($version, $totaltime)
{
	if(!$_POST['excel_export'])
	{
		...
		else print "<HR><FONT SIZE=2 COLOR=GREY>v.$version  | <I>executed in <FONT SIZE=2 COLOR=#FF7777>$totaltime</FONT> seconds</I></FONT>";
	} 
}
...
//--------------------------------------------------------------------------------------------------------------
function alert($text)
//	return some text as in an alert box on the users device
{
	print "
