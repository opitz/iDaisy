...
$version_caf = "120810.1";			// show staff with stint only by default, add option to show staff without stint
$version_caf = "120822.1";			// added summary in staff list
$version_caf = "120911.1";			// changed code that only Administrators or higher will see post details
...
//			$row['St Superv'] = $supervision_stint;
		...
		if($include_zero_stint OR !$ay_id ) $new_table[] = $row;
...
//	if still no e_id get webauth user id
...
//			if($is_editor) $row['DAISY'] = "<A HREF=https://daisy.socsci.ox.ac.uk/post/$p_id/edit TARGET=NEW>Edit</A>";
		...
		if(current_user_is_in_DAISY_user_group("Administrator"))
		{
