...
//--------------------------------------------------------------------------------------------------
function show_programmes_by_year1($au_id, $given_ay_id)
//	Show Programmes of which a unit is part of
{
	$given_ay_id = $_GET['ay_id'];												// get seleced academic_year_id
...
#			dp.title AS 'Degree Programme',
			...
			WHERE audp.assessment_unit_id = $au_id ";
			if($given_ay_id > 0) $query = $query."AND ay.id = $ay_id ";
			$query = $query."
	...
	if (!$title_printed) print "No related Degree Programme!<P>";
}
...
//--------------------------------------------------------------------------------------------------
function show_enrollment_details_by_year0($au_id, $given_ay_id)
//	print teaching details for a given Assessment Unit ID
{
	$title_printed = FALSE;
