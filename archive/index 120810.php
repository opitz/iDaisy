...
//	get the users webauth code as identifier for further actions
$webauth_code = $_GET["webauth"];							// get WebAuth code
if(!$webauth_code) $webauth_code = $_GET["webauth_code"];	// get WebAuth code 2nd try
if(!$webauth_code) $webauth_code = $_POST['webauth_code'];	// get WebAuth code 3rd try

//	only supervisors and overseers are allowed to get reports as someone else otherwise get their own webauth code
if(!current_user_is_in_DAISY_user_group("Super-Administrator")) $webauth_code = $_SERVER['REMOTE_USER'];
//	finally if there is no logged in user - we are most likely on the dev server now - get a predefined webauth code
if(!$webauth_code) $webauth_code = is_user();
