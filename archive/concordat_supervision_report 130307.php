...
//	calculate the Appointment Split for a given supervisor and academic year and use this to compute the Actual Stint
{
	...
	foreach ($table AS $row)
	{
		...
		if(in_array($st_d_id , $post_dept_ids) AND $row['Type'] != 'DPhil')
		{
			if($e_d_id == $st_d_id) $row['Actual Stint'] = number_format($row['Supervision Stint'] * $row['Supervision Percent'] / 300 * $factor, 2);
			else $row['Actual Stint'] = number_format(0,2);
		}
		else $row['Actual Stint'] = number_format($row['Supervision Stint'] * $row['Supervision Percent'] / 300 * $app_split * $factor, 2);
		$new_table[] = $row;
	}
	return $new_table;
}
