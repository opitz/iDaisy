...
//	display the option checkboxes
	...
	print new_column(0);
		print "Show Stint Details for all academics:";
	print new_column(0);
...
//		$excel_title = "Query_".str_replace(" ", "_", $department['department_name']);
	...
	if($_POST['show_post'])
	{
		...
		$e_posts = show_post_details($e_id);
dprint($e_posts);	
	if($_POST['show_teaching'] AND $e_posts) show_teaching_details_by_year($e_id, $e_posts);
	if($_POST['show_supervision']) show_supervision_details_by_year($e_id, $e_posts);
	if($_POST['show_leave']) show_leave_details_by_year($e_id);
...
//	print "<H3>Employee Details</H3>";
	if($_POST['show_stint_details_list']) print "<H3><A HREF=$his_page?e_id=$e_id&ay_id=$ay_id&show_post=1>".$e_data['title']." ".$e_data['forename']." ".$e_data['initials']." ".$e_data['surname']."</A></H3>";
...
//			if($is_editor) $row['DAISY'] = "<A HREF=https://daisy.socsci.ox.ac.uk/post/$p_id/edit TARGET=NEW>Edit</A>";
		...
		if($_POST['show_post'] AND (current_user_is_in_DAISY_user_group("Administrator") OR !current_user_is_in_DAISY_user_group("Editor")))
		{
...
function staff_switchboard0()
{
	...
	print "<TD WIDTH=400 ALIGN=LEFT BGCOLOR=LIGHTGREY>".reload_ay_button()."</TD>";

	print "<TD WIDTH=400 ALIGN=CENTER>".teaching_button()."</TD>";
	...
	print "<TD WIDTH=200 ALIGN=LEFT>".office_button()."</TD>";
	print "<TD WIDTH=200 ALIGN=LEFT>".committee_button()."</TD>";
...
/*
	...
	print "<TD WIDTH=400 ALIGN=LEFT BGCOLOR=LIGHTGREY> </TD>";
