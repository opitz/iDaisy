...
#			e.fullname AS 'Full Name',
			...
			IF((SELECT COUNT(*) FROM Post p2 INNER JOIN Department d2 ON d2.id = p2.department_id WHERE p2.employee_id = e.id AND d2.department_code LIKE '$department_code%') > 0, CONCAT('<A HREF=index.php?e_id=', e.id, '&ay_id=$ay_id>',e.fullname,'</A>'),CONCAT('<A HREF=index.php?e_id=', e.id, '&ay_id=$ay_id&borrowed=1&department_code=$department_code>',e.fullname,'</A>')) AS 'Full Name',
...
//				if($include_borrowed_staff) $query = $query."AND (d.department_code LIKE '$department_code%' OR od.department_code LIKE '$department_code%')";
	...
	{		
		...
		staff_query_form($department_code, $ay_id, $_SERVER["PHP_SELF"]); // this script will call this page again
		print "<HR>";
	}
...
//	select the data
		$query = "
			...
			CONCAT('<A HREF=index.php?tc_id=',tc.id,'&ay_id=$ay_id>',tc.subject,'</A>') AS 'Subject',
...
//----------------------------------------------------------------------------------------
function get_teaching_stintx0($e_id, $ay_id, $d_id)
...
//	select the teaching data
	$query = "
		SELECT DISTINCT
		SUM(ROUND((ti.sessions * ti.percentage / 100 * tctt.stint),2)) AS 'Stint'
