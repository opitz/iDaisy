...
//	display the option checkboxes
	...
	print new_column(190);
		 print "Exclude Assessment <BR>Units without teaching:";
	print new_column(0);
		if ($_POST['exclude_no_teaching'])  print "<input type='checkbox' name='exclude_no_teaching' value='TRUE' checked='checked'>";
		else print "<input type='checkbox' name='exclude_no_teaching' value='TRUE'>";
	print end_row();
...
function show_unit_list1()
{
	...
	$report_title = "Department Teaching Report";
	if ($excel_export)
	{
		excel_header($report_title);
	}
	else
	{
...
LEFT JOIN TeachingComponentTypeTariff tctt ON tctt.teaching_component_type_id = tct.id AND tctt.academic_year_id = tcau.academic_year_id
...
LEFT JOIN Post p ON p.employee_id = e.id AND (p.startdate < t.enddate AND IF(YEAR(p.enddate) > 1980, p.enddate > t.startdate, 1=1)) 
	";
	$query = $query . "
...
AND au_d.department_code LIKE '3C11%' 


GROUP BY au.assessment_unit_code
...
//--------------------------------------------------------------------------------------------------------------
function amend_unit_list($table)
{
	$ay_id = $_POST['ay_id'];
	$new_table = array();
...
//	get the student norms
		...
		$row['Norm Student Provision'] = $student_norm_data['Stint'];
...
//		$dept_norm_data = get_dept_norm($au_id, $ay_id);
		$dept_norm_data = get_dept_norm($au_id, $ay_id, $row['Students Entered']);
		$row['Norm Dept Provision'] = $dept_norm_data['Stint'];
...
//	if(!$department_code) $query = $query."CONCAT(d.department_code, ' - ', d.department_name) AS 'Department',";
	$query = $query."CONCAT(d.department_code, ' - ', d.department_name) AS 'Department',";
	$query = $query."
		au.assessment_unit_code AS 'Code',
...
//		$au_id = $row['AU_ID'];
		...
		if($_POST['show_teaching_details_list'])
		{
			...
			{
				if(get_unit_teaching_stint($au_id, $ay_id) > 0) 
				{
...
//	if an academic year was selected do some more...
			if($ay_id > 0) 
			{
				...
				$au_students = count_au_students($au_id, $ay_id);
//				$dept_norm = get_dept_norm($au_id, $ay_id);
				$dept_norm = get_dept_norm($au_id, $ay_id, $au_students);
				...
				$row['Students Entered'] = $au_students;
			...
			if ($_POST['exclude_no_teaching'])  
			{
				if($row['Actual Dept Provision - Stint'] > 0) $new_table[] = $row;
			} 
			else $new_table[] = $row;
		}
	}		
...
//		$excel_title = "Query_".str_replace(" ", "_", $department['department_name']);
		$excel_title = "Dept_Teaching";
		if(!$_POST['show_teaching_details_list']) excel_header($excel_title);
	}
...
//	show_unit_specs($au_data);
	...
	if($_POST['show_unit_teaching'] OR $_POST['show_teaching_details_list']) show_unit_teaching_details_by_year();
...
//	Show Programmes of which a unit is part of
{
	...
	print "Provided by ".get_au_dept($au_id)."<P>";
...
//	select the data
	$query = "
		...
		if($ay_id > 0) $query = $query."AND audp.academic_year_id = $ay_id ";
		$query = $query."
...
//dprint($query);
	...
	if($table)
	{
		...
		print "No related Degree Programme!";
		
	if(dt_flag_is_set($au_id)) print "<BR />Available for Doctoral Training";
	print "<P>";
}

//--------------------------------------------------------------------------------------------------
function dt_flag_is_set($au_id)
//	check if the AU is ment to be available for Doctoral Training
{
	$query = "SELECT * FROM AssessmentUnit WHERE id = $au_id ";
	$restable = get_data($query);
	return $restable[0]['doctoral_training'];
}
...
//	define column width in table
	$table_width = array('Term' => 50, 'Department' => 300, 'Subject' => 350, 'Plan Sess.' => 50, 'Sess. Stint' => 50, 'Sess. Hours' => 50, 'Given Sess.' => 50, 'Type' => 350, 'Unique TC Students' => 50, 'Assessment Units (Students)' => 350);
...
$query = $query . "
CONCAT('<A HREF=$this_page?tc_id=',tc.id,'&ay_id=$given_ay_id&department_code=$department_code>',tc.subject,'</A>') AS 'Teaching Component',
tct.title AS 'Teaching Component Type',
tcau.capacity AS 'Cap.',
tc.sessions_planned AS 'Plan Sess.',
tctt.stint AS 'Sess. Stint',
tctt.hours AS 'Sess. Hours',
t.term_code AS 'Term',
...
INNER JOIN TeachingInstance ti ON ti.teaching_component_id= tc.id AND ti.academic_year_id = $ay_id
...
//	show a link to DAISY for selected user groups
...
//	do the sums
			if($sum_stint > 0)
			{
//	1st insert a blank line
				$row =fill_line($row, '');
				$row['Teaching Component'] = '<HR>';
				$row['Stint'] = '<HR>';
				$row['Hours'] = '<HR>';
				$new_table[] = $row;
//	now print the sum
				$row['Teaching Component'] = "<B>Total:</B>";
				$row['Stint'] = "<U><B>$sum_stint</B></U>";
				$row['Hours'] = "<U><B>$sum_hours</B></U>";
				$new_table[] = $row;
			}
...
st.id AS 'ST_ID',
CONCAT(st.surname, ', ', st.forename) AS 'Student',
IF(sau.source = 'SES', 'SES', 'OSS') AS 'Enrolled via'
...
// returns number of students enrolled to an teaching component in a given year
{
	$query = "
...
//--------------------------------------------------------------------------------------------------------------
function get_student_norm0($au_id, $ay_id)
...
//--------------------------------------------------------------------------------------------------------------
function get_all_tc_students0($tc_id, $ay_id)
