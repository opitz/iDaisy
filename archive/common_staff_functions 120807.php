...
//--------------------------------------------------------------------------------------------------------------
function show_staff_list($webauth_code)
{
...
#			e.fullname AS 'Full Name',
			...
			CONCAT('<A HREF=index.php?e_id=', e.id, '&ay_id=$ay_id>',e.fullname,'</A>') AS 'Full Name',
			e.opendoor_employee_code AS 'Employee Number',
#			CONCAT('<A HREF=academic_details.php?e_id=', e.id, '&ay_id=$ay_id>',e.opendoor_employee_code,'</A>') AS 'Employee Number',
...
//				if($include_borrowed_staff) $query = $query."AND (d.department_code LIKE '$department_code%' OR od.department_code LIKE '$department_code%')";
	...
	if(!$excel_export)
	{
		$title = "<FONT face='Arial'><B>DAISY<I>report</I> | Academic Report</B></FONT> <BR />
			<EM> for user <B>$webauth_code</B></EM> | <EM>connected to <B>$db_name</B></EM>";

		print $title."<BR>";
		if ($date) print "Run on: $date<P>";
		print "<HR>";
//		if ($query) print_export_button($query, $ay_id);
		if ($query) print_export_button($parameter);
		staff_query_form($webauth_code, $department_code, $ay_id, $_SERVER["PHP_SELF"]); // this script will call this page again
		print "<HR>";
	}
...
//						$row['StOb'] = $stob - $stob * $leave_reduction_factor;
				...
				} else
				{
					...
					$supervision_stint = get_supervision_stint($e_id, $ay_id, $dept_id);
				}
...
//-----------------------------------------------------------------------------------------
function show_staff_details($webauth_code)
{
...
//	$title = "<FONT face='Arial'><B>DAISY<I>info</I> | Academic Details</B></FONT> for <B>$e_fullname</B><BR />
//		<EM> requested by <B>$webauth_code</B></EM> | <EM>connected to <B>$db_name</B></EM><P>";
	$title = "<FONT face='Arial'><B>DAISY<I>info</I> | Staff Details</B></FONT> for <B>$e_fullname</B><BR />
		<EM> connected to <B>$db_name</B></EM>";

	print $title;
	if ($date) print " | <EM>Run on: $date</EM><P>";
	if ($ay_id > 0)
...
#		ssdc.ssd_category_name AS 'SSD Category'
		...
		foreach($table AS $row)
		{
			if($row['Status'] == 'ACTV')
			{
				...
				if($ay_id > 0)
				{
					...
					$teaching_stint = get_teaching_stint($e_id, $ay_id, $new_row['D_ID']);
					$row['T Stint'] = $teaching_stint;
					$supervision_stint = get_supervision_stint($e_id, $ay_id, $new_row['D_ID']);
...
//	select the data
//	NOT TRUE!!! ATTENTION: for TC with relations to more than one AU - except the zombie AU - we need to divide the sum of stint by the number of AUs - except the zombie AU!
		$query = "
			SELECT DISTINCT
			tc_d.id AS 'TC_D_ID',
			ti.id AS 'TI_ID',
			tc.id AS 'TC_ID',
			#au_d.id AS 'AU_DEPT_ID',
			...
			t.term_code AS 'Term',
			ti.sessions AS 'Sess.',
#			SUM(ti.sessions) AS 'SumSess.',
			ti.percentage AS 'Perc.',

#NOT TRUE!	ATTENTION: for TC with relations to more than one AU - except the zombie AU - we need to divide the sum of stint by the number of AUs - except the zombie AU!
#			ROUND((ti.sessions * ti.percentage / (100 * IF((SELECT COUNT(*) FROM TeachingComponentAssessmentUnit tcau2 WHERE tcau2.teaching_component_id = tc.id AND tcau2.academic_year_id = t.academic_year_id) > 1 ,(SELECT COUNT(*) FROM TeachingComponentAssessmentUnit tcau2 WHERE tcau2.teaching_component_id = tc.id AND tcau2.academic_year_id = t.academic_year_id) - 1, 1)) * tctt.stint),2) AS 'Old Stint'
			ROUND((ti.sessions * ti.percentage / 100 * tctt.stint),2) AS 'Stint'

#			CONCAT('<A HREF=https://daisy.socsci.ox.ac.uk/teaching_instance/',ti.id,'/edit TARGET=NEW>Edit</A>') AS 'DAISY'
...
#			LEFT JOIN StudentAssessmentUnit sau ON sau.assessment_unit_id = au.id AND sau.academic_year_id = tcau.academic_year_id 
...
//	ATTENTION! The following query will use the component type stored in the NON TIME AWARE table 'Teaching Component'
//	This script is intended to be used only while the flaw in the interface for the time aware table 'TeachingComponentAssessmentUnit' has been solved!
/*
		$query0 = "
			SELECT DISTINCT
			tc_d.id AS 'TC_D_ID',
...
//	sum up the stints earned by department
			...
			if(in_array($tc_d_id, $e_post_d_id)) $row['Stint Calc'] = 'Post';
			else 
				if(count($e_posts)>1) $row['Stint Calc'] = 'Other Split';
				else $row['Stint Calc'] = 'Other';
				
			if(in_array($tc_d_id, $e_post_d_id)) $sum_stint["$dept"] = $sum_stint["$dept"] + $row['Stint'];
			else
			{
				foreach($e_posts AS $e_post)
				{
...
//----------------------------------------------------------------------------------------
function get_teaching_stint($e_id, $ay_id, $d_id)
