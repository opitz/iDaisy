...
//	
//	Last Update by Matthias Opitz, 2012-12-04
//
//=============================================================================================================
$version = '121204.1';
//==============================================< Here we go! >====================================================
...
//	check if publication exists where used and add it if not
		...
		if($title)
		{
			$publication_id = get_publication_id($row['Title of Output'], $d_id);
			if($journal_id < 1) 
			{
//				unset($journal_id);
//				$journal_id = null;
...
// ==============================================< Publications >==================================================
//--------------------------------------------------------------------------------------------------
function get_publication_id0($title, $d_id)
...
//--------------------------------------------------------------------------------------------------
function get_publication_id($title, $d_id)
// return the ID for a given publication if it exists
{
	$query = "
		SELECT id, title, UPPER(title) AS 'uppertitle' 
		FROM Publication
		WHERE UPPER(title) = '".mb_strtoupper(addslashes($title), 'UTF-8')."' 
			AND department_id = $d_id
		";

	$table = get_data($query);
	$record = $table[0];
if($title != $record['title']) dprint("PHP: $title<BR />SQL: ".$record['title']);
//if(mb_strtoupper(addslashes($title), 'UTF-8') != addslashes($record['uppertitle'])) dprint("PHP: ".mb_strtoupper(addslashes($title), 'UTF-8')."<BR />SQL: ".$record['id']." | ".$record['title']);
//dprint($record['uppertitle']);
	return $record['id'];
//		WHERE title = '".utf8_decode($assoc_data['Title of Output'])."' 
}
...
//	if($assoc_data['Pending Publication?']) $publication_status_id = 10; // refers to 'REF pending'
	...
	$query = "
		...
		VALUES (
			...
			'".addslashes(htmlentities($assoc_data['Title of Output']))."',
			'$output_type_id',
			'$sqldate',
...
// add a journal with $title for department with $d_id
{
	print "Output Type '$output_type' does not exist!\n";
	return FALSE;
}
