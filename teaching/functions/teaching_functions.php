...
//	Last changes: Matthias Opitz --- 2013-03-21
//
//==============================================================================================================
...
//	print write_post_vars_html();
	...
	print start_row(0) . "Assessment Unit Code:" . new_column(0) . "<input type='text' name = 'au_code_q' value='".$_POST['au_code_q']."' size=10>" . end_row();		
...
//--------------------------------------------------------------------------------------------------------------
function amend_teaching_summary($table)
{
	$ay_id = $_POST['ay_id'];
	$new_table = array();
	if($table) foreach($table AS $row)
...
//--------------------------------------------------------------------------------------------------------------
function xxxinsert_sub_totals0($table)
//	inserts a sub-total row for each assessment unit with more than one row
{
	$ay_id = $_POST['ay_id'];
	$new_table = array();
...
//	get assessment unit stint and hours
				$totals_row['Teaching Component name'] =  number_format(au_stint($ay_id, $prev_au_id),2);
				$totals_row['Actual AU Hours'] =  number_format(au_hours($ay_id, $prev_au_id),2);

			}
			else
			{
//print_r($blank_row); print "<HR>";
				$totals_row['AU Code'] = "<B><U>Sub-Total</U></B>";
				$totals_row['Assessment Unit / PGR Module'] = "<B><U>$prev_au_title</U></B>";
//	get assessment unit stint and hours
				$totals_row['Actual AU Stint'] =  "<B><U>".number_format(au_stint($ay_id, $prev_au_id),2)."</U></B>";
				$totals_row['Actual AU Stint Share'] =  "<B><U>".number_format(au_stint_share($ay_id, $prev_au_id),2)."</U></B>";
				$totals_row['Actual AU Hours'] =  "<B><U>".number_format(au_hours($ay_id, $prev_au_id),2)."</U></B>";
			}
...
// show a department column only when used on divisional level or above
	...
	$query = $query."
		...
		IF((SELECT COUNT(*) FROM AssessmentUnitDegreeProgramme WHERE core_option = 'Core' AND assessment_unit_id = au.id AND academic_year_id = $ay_id) > 0, 'Core', 'Option') AS 'Core / Option',
		IF((SELECT COUNT(*) FROM AssessmentUnitDegreeProgramme WHERE unit_type LIKE 'UGRAD%' AND assessment_unit_id = au.id AND academic_year_id = $ay_id) > 0, 'UGRAD', 'PGRAD') AS 'Unit Type'
	...
	$query = $query."
		WHERE 1=1
		AND au.title LIKE '%".$_POST['au_title_q']."%'
		AND au.assessment_unit_code LIKE '%".$_POST['au_code_q']."%'
		AND d.department_code LIKE '".$_POST['department_code']."%'
		
#		ORDER BY d.department_code, au.assessment_unit_code
		ORDER BY
			d.department_name,
			IF((SELECT COUNT(*) FROM AssessmentUnitDegreeProgramme WHERE unit_type LIKE 'UGRAD%' AND assessment_unit_id = au.id AND academic_year_id = $ay_id) > 0, 'UGRAD', 'PGRAD') DESC,
			IF((SELECT COUNT(*) FROM AssessmentUnitDegreeProgramme WHERE core_option = 'Core' AND assessment_unit_id = au.id AND academic_year_id = $ay_id) > 0, 'Core', 'Option') ASC,
			d.department_code ASC,
			au.assessment_unit_code ASC
	";
...
//--------------------------------------------------------------------------------------------------------------
function xxxamend_stint_per_student($table)
{
	$ay_id = $_POST['ay_id'];
	$new_table = array();
	if($table) foreach($table AS $row)
...
//--------------------------------------------------------------------------------------------------------------
function xxxstudent_ses_enrolment_report_by_dept()
//	shows the Doctoral Training Courses (AU) into which students of a given department or division are enrolled
{
	$table = xxxget_own_students_per_ses_course_dept();	// get all students of a selected department or division that are enrolled into any graduate course
...
FORMAT(SUM(sdp.student_id/sdp.student_id * dpd.percentage / 100),1) AS 'AU Students'
...
//-------------------------------------------------------------------------------------------------------------
function xxxget_own_students_per_ses_course_dept()
{
	$ay_id = $_POST['ay_id'];								// get academic_year_id
	$department_code = $_POST['department_code'];		// get department code
...
//--------------------------------------------------------------------------------------------------------------
function xxxis_part_of_programme($au_id, $ay_id)
...
function xxxis_ugrad($au_id, $ay_id)
// returns TRUE if the given assessment unit has a UGRAD unit type with at least one degree programme
{
	$query = "
		SELECT COUNT(*) AS 'COUNT'
...
//--------------------------------------------------------------------------------------------------------------
function xxxis_pgrad($au_id, $ay_id)
...
//--------------------------------------------------------------------------------------------------------------
function xxxis_core($au_id, $ay_id)
...
//--------------------------------------------------------------------------------------------------
function xxxunit_has_teaching($au_id, $ay_id)
...
//--------------------------------------------------------------------------------------------------
function xxxunit_has_enrollment($au_id, $ay_id)
...
//--------------------------------------------------------------------------------------------------
function xxxstudent_is_enrolled($st_id, $ay_id)
//	checks if  a given student enrolled at all (for a given academic year)
{
	$query = "
		SELECT * 
